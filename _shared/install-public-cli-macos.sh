...
if [[ "${1:-}" == "" || "${2:-}" == "" || "${3:-}" == "" ]]; then
  cat <<'USAGE'
Usage:
  ./install-public-cli-macos.sh <owner/repo> <asset_name> <binary_name> [install_dir]

Example:
  ./install-public-cli-macos.sh cli/cli gh_2.96.0_macOS_amd64.tar.gz gh "$HOME/.local/bin"
USAGE
  exit 1
fi
